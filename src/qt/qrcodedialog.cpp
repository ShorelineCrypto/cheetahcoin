...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CHTA unit input in lnReqAmount, we generate the URI with CHTA as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(CheetahcoinUnits::format(CheetahcoinUnits::CHTA, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
