...
#if defined(NDEBUG)
# error "Cheetahcoin cannot be compiled without assertions."
#endif
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s CHTA additional fees, %d delta bytes\n",
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check timestamp - Enforce v1.10.x timestamp 4 minute rule after v1.10.x hard fork 
    if (block.GetBlockTime() > nAdjustedTime + 1 * 4 * 55)
    ...
    if ((nHeight > 1408590) && (block.GetBlockTime() > nAdjustedTime + 1 * 1 * 30))
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (orphanTx.wit.IsNull() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/cheetahcoin/cheetahcoin/issues/8279 for details.
                            assert(recentRejects);
