...
uint256 hashGenesisBlock("0x0000000090ae6bab6c2abd99179a7632b84f286f876def641dd35c3221eee7be");
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uCHTA
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
 ...
 // Premine 1% in first 10 blocks
    if(nHeight <= 10)
        nSubsidy = 21000000 * COIN; // first 10 blocks obtain 21million per block reward
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        ...
        // v1.10.x randomSpike fork after block height 666136
...
void ThreadScriptCheck() {
    RenameThread("cheetahcoin-scriptch");
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    if (!fIsInitialDownload)
    {
        ...
        if (nUpgraded > 100/2)
            ...
            strMiscWarning = _("Warning: 50% unknown/higher versions detected on 100 blocks!");
    }
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // Enforce timestamp 4 minute rule, v1.10.x fork on block height 727770
        if ((nHeight > 727770) && (GetBlockTime() > GetAdjustedTime() + 1 * 4 * 55))
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000000053b9649341c2628edb2516993a9dd54ac5ffeb7a68ef6552d6b6ac7b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Cointelegraph 11/06/2020 Fed calling for more stimulus is code for buy Bitcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a745e457e294def66d9a71fcf99e182a9073362b0d5c7c85dce406f63d3048465f3a1a63d534f7181552fecd64efa6b2d7e9c7469890436fd81c5d660375c4e6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1605846920;
        block.nBits    = 0x1d00ffff;
        block.nNonce   = 1588083351;

        if (fTestNet)
        {
            block.nTime    = 1605899705;
            block.nNonce   = 2509547857;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2886b81c1844a342c3c17baf1abc809f37d7b41e375cdfef294be6812f713f04"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xca, 0xb3, 0xd4 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CheetahcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CheetahcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CheetahcoinMiner(CWallet *pwallet)
{
    printf("CheetahcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cheetahcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CheetahcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CheetahcoinMiner terminated\n");
        throw;
    }
}

void GenerateCheetahcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CheetahcoinMiner, pwallet));
}
